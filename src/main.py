...
from algs import policy_rct
from algs import policy_adaptiveenrichment
from algs import policy_futilitystopping
from algs import policy_greedybayesocp
from algs import policy_bayesocp
...
hyper['envr_costf'] = Partial(lambda xset: 1 / np.sum(hyper['envr_prop'], where=xset)**.1)
hyper['envr_rwrdf'] = Partial(lambda xset: 1000 * np.sum(hyper['envr_prop'], where=xset)**.1)

###

hyper['test0'] = dict(size=0, mean=0., stat=-1)
hyper['test_horizon'] = 600
hyper['test_threshold'] = norm.ppf(.95)

@jax.jit
def test_update(test, y):
    test['size'] = test['size'] + np.where(test['stat'] < 0, 1, 0)
    test['mean'] = test['mean'] + np.where(test['stat'] < 0, (y - test['mean']) / test['size'], 0.)
    test['stat'] = np.where(test['size'] == hyper['test_horizon'], 
            test['mean'] / np.sqrt(1 / test['size']) > hyper['test_threshold'], -1)
    return test

@jax.jit
def test_predict(test, envr_mean):
    _mean = (hyper['test_threshold'] * np.sqrt(hyper['test_horizon']) - test['mean'] * test['size']) / (hyper['test_horizon'] - test['size'])
...
hyper['post0'] = dict(
        size=np.ones(hyper['n'], dtype=int) * 10,
        mean=np.ones(hyper['n']) * .1)
...
def _simulate(arg0, arg1, policy, envr_mean):
    ...
    _xset = np.where(np.logical_and(test['stat'] == 0, np.all(_xset == xset)), hyper['xset_nil'], _xset)
...
def simulate(key, policy, envr_mean):
    _, (xsets, tests, posts, infos) = jax.lax.scan(Partial(_simulate, policy=policy, envr_mean=envr_mean), 
            (hyper['xset_all'], hyper['test0'], hyper['post0']), jax.random.split(key, hyper['horizon']))
    return xsets, tests, posts, infos

###

hyper['policy_threshold'] = .80
