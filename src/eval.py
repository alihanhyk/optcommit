...
freq = dict(green=0.0, amber=0.0, red=0.0)
utility = dict(all=list(), green=list(), amber=list())
success = dict(all=list(), green=list(), amber=list())

envrs_type = list()
for envr_mean in envrs_mean:
    succ = dict(green=None, amber=None, red=0.0)
    util = dict(green=None, amber=-np.inf, red=0.0)
...
res = {(typ, alg, met): [list() for _ in range(fold_n)] for typ in key_typs for alg in key_algs for met in key_mets}
...
for k in tqdm.trange(fold_n * fold_size):
    ...
    for alg in key_algs:
        ...
        _util = hyper["envr_rwrdf"](xsets[_time]) if tests["stat"][_time] == 1 else 0.0
...
res = {
    (typ, alg, met): np.array([np.nanmean(np.array(res[(typ, alg, met)][i])) for i in range(fold_n)])
    for typ in key_typs
    for alg in key_algs
    for met in key_mets
}
...
values = [
    [
        ...
        "",
        "",
        f'{success["all"]*100:.1f}%',
        "",
        "",
        f'{utility["green"]:.1f}',
        "",
        "",
        f'{success["green"]*100:.1f}%',
        "",
        "",
        f'{utility["amber"]:.1f}',
        "",
        "",
        f'{success["amber"]*100:.1f}%',
        "",
        "",
        "",
        "",
...
headers = [
    f'{typ}{f" ({freq[typ]*100:.1f}%)" if typ != "all" else ""}\n{met}' if met == "utility" else f"\n{met}"
    for typ in key_typs
    for met in key_mets
]
